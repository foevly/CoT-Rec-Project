...
def parse_args():
    ...
    parser.add_argument('--verbose', type=int, default=100, help="Interval for verbose output_64.")
...
if __name__ == '__main__':
    ...
    if args.iteminit == 'feature':
        ...
        print(f'pretrain_{args.embedding_dim}/{args.dataset_name}_feature.npy已加载')
    else:
        pretrained_embeddings = torch.from_numpy(np.load(f'pretrain_{args.embedding_dim}/{args.dataset_name}_caption.npy'))
        print(f'pretrain_{args.embedding_dim}/{args.dataset_name}_caption.npy已加载')
    print(pretrained_embeddings.requires_grad)
    print("pretrained_embeddings.shape[1]: " + str(pretrained_embeddings.shape[1]))
    print("pretrained_embeddings.shape[0]: " + str(pretrained_embeddings.shape[0]))
    print("dataset.num_items: " + str(dataset.num_items))
    if pretrained_embeddings.shape[0] == dataset.num_items + 1:
        pretrained_embeddings = pretrained_embeddings[:dataset.num_items, :]
    elif pretrained_embeddings.shape[0] == dataset.num_items:
        pass
    else:
        raise ValueError(
            f"预训练行数={pretrained_embeddings.shape[0]} 与 num_items={dataset.num_items} 不匹配（既不是 N 也不是 N+1）"
        )
    model = SASRec(pretrained_embeddings, dataset.num_items, args.embedding_dim, args.max_length, args.num_layers, dropout=args.dropout).to(args.device)
