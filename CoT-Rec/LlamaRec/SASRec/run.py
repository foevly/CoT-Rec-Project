...
def parse_args():
    ...
    parser.add_argument('--topk_list', type=int, nargs='+', default=[10], help="List of top-k values for evaluation.")
    ...
    args.filepath = f'../datasets/processed/{args.dataset_name}.csv'
...
if __name__ == '__main__':
    ...
    model = SASRec(dataset.num_items, args.embedding_dim, args.max_length, args.num_layers, dropout=args.dropout).to(args.device)
    ...
    for epoch in range(1, args.num_epochs + 1):
        ...
        if ndcg_valid[max(args.topk_list)] >= best_ndcg_valid:
            ...
            best_epoch = epoch
            torch.save(model, f"checkpoint/{args.dataset_name}.pth")
            with open(f'checkpoint/{args.dataset_name}_rec_list_valid.pkl', 'wb') as f:
                pickle.dump(rec_list_valid, f)
            with open(f'checkpoint/{args.dataset_name}_rec_list_test.pkl', 'wb') as f:
                pickle.dump(rec_list_test, f)
        else:
            patience -= 1
            if patience == 0:
                break
    with open(f"checkpoint/{args.dataset_name}.log", 'a') as f:
